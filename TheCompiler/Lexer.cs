...
namespace TheCompiler
{
    internal class Lexer
    {
        ...
        private void SemanticProgram19(char c)
        {
            ...
            if (!storage.memory.ContainsKey(number.ToString()))
            {
                storage.memory.Add(number.ToString(), new int[1] { number });
            } 
        }
        ...
        public void Analyze(string program)
        {
            ...
            for (int i = 0; i < program.Length; i++, character++)
            {
                ...
                {   
                    storage.error = "Ошибка при работе лексического анализатора в строке: " + (line + 1) + "символ: " + (character + 1);
                    return;
                }
            }
        }
